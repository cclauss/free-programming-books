...
def lint_file(path: str, cfg: dict) -> list:
    ...
    for idx, line in enumerate(lines, 1):
        ...
        if book_item:
            ...
            # If the list item is just a link like the link in the section "### Index"
            # of the .md files (i.e., [Title](url))
        ...
        if (
            ...
            issues.append(
                f"::{sev['author_meta'].lower()} file={path},line={idx}::RTL author "
                f"'{author.strip()}' followed by LTR meta '{meta}' may need '&rlm;' "
                "after author."
            )
        ...
        for part, raw_text in [("title", title), ("author", author), ("meta", meta)]:
            ...
            for segment_text, segment_direction_ctx in segments:
                ...
                # In the following block of code, it's checked if the segment is entirely enclosed in parentheses or
                # brackets.
                ...
                if m_bracket:
                    ...
                    # Check for pure RTL: contains RTL chars AND no LTR chars (using [A-Za-z0-9] as a proxy for common
                    # LTR chars)
                    is_pure_rtl_inner = (
                ...
                # Skip if it's inline code (i.e., `...`) or already contains directionality markers
                # (e.g., &rlm; or &lrm;)
                ...
                if rtl_char_re.search(s) and re.search(r"[A-Za-z0-9]", s):
                    ...
                    if disp != s:
                        issues.append(
                            ...
                            f"::BIDI mismatch in {part}: the text '{s}' is displayed "
                            "as '{disp}'"
                        )

                # If the segment context is LTR, there is no need to check LTR keywords and LTR symbols
                # that might need directionality markers, so we can skip the next checks and move on to the next line
                # of the file
                if segment_direction_ctx != "rtl":
                    continue

                # Skip keyword and symbol checks for titles of link-only items (e.g., in the Index section of markdown
                # files)
                    ...
                    for sym in symbols:
                        if sym in s and not any(m in s for m in LRM):
                            issues.append(
                                ...
                                f"::Symbol '{sym}' in {part} '{s}' may need trailing "
                                "'&lrm;' marker."
                            )
                    ...
                    for kw in filtered_keywords:
                        if kw in s and not any(m in s for m in RLM):
                            issues.append(
                                ...
                                f"::Keyword '{kw}' in {part} '{s}' may need trailing "
                                "'&rlm;' marker."
                            )
        ...
        for p_scan_arg in args.paths_to_scan:
                        ...
                        if fn.lower().endswith(".md"):
                            ...
                            for issue_str in issues_found:
                                ...
                                if issue_str.startswith(
                                    "::error"
                                ) or issue_str.startswith("::warning"):
                                    errs += 1
            ...
            # If the path is a Markdown file, lint it directly
                ...
                for issue_str in issues_found:
                    ...
                    # Count all errors/warnings for reporting/debugging purposes
                    if issue_str.startswith("::error") or issue_str.startswith(
                        "::warning"
                    ):
                        errs += 1
